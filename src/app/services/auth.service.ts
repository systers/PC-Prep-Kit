...
export class AuthService {
  ...
  public loginUser(body: Object): Observable<any> {
    ...
    return this._apiservice.post(this._loginAuth, body).pipe(
      tap(res => console.log(' Login requests received: Current Token: ' + localStorage.getItem('pcprepkitUser'))),
      map(res => {
        if (res.info) {
          throw res;
        }
  ...
  public logout() {
    ...
    return this._apiservice.get(this._logoutAuth).pipe(
      tap(res => console.log(' Logout requests received: Current Token: ' + localStorage.getItem('pcprepkitUser'))),
      map((res) => res));
  }
}
